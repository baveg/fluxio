...
export const setMs = (d: DateLike, v: number) =>
  updateDate(d, (d) => d.setMilliseconds(v));
...
export const setWeekDay = (d: DateLike, v: number) =>
...
export const setMonth = (d: DateLike, v: number) =>
  updateDate(d, (d) => d.setMonth(v));
...
export const addMonth = (d: DateLike, v: number = 1) =>
  setMonth((d = toDate(d)), getMonth(d) + v);

/** Add years - addYear('2025-02-09', 2) -> 2027-02-09 */
export const addYear = (d: DateLike, v: number = 1) =>
  setYear((d = toDate(d)), getYear(d) + v);
...
export const dayIndexToName = (i: number | null | undefined): DayName | "" =>
  isUInt(i) ? DAY_NAMES[i] || "" : "";

/** 0 -> "Dim", 1 -> "Lun", 2 -> "Mar", ... */
export const dayIndexToShort = (i: number | null | undefined): DayShort | "" =>
...
export const formatDay = (d?: DateLike): DayName | "" =>
...
export const isSameYear = (a: DateLike, b: DateLike): boolean =>
  getYear(a) === getYear(b);
...
export const isTomorrow = (d: DateLike): boolean =>
  isSameDay(d, addDay(serverDate(), 1));

/** Check if date is yesterday - isYesterday(addDay(new Date(), -1)) -> true */
export const isYesterday = (d: DateLike): boolean =>
  isSameDay(d, addDay(serverDate(), -1));
