import { computeStyle, Css } from '@fluxio/core/html/css';
import { stopEvent } from '@fluxio/core/html/stopEvent';
import type { Vector2 } from '@fluxio/core/types/Vector';
import type { Vector4 } from '@fluxio/core/types/Vector';
...
import { getEventXY } from '@fluxio/core/html/getEventXY';
...
import { clampVector, VECTOR2_ZERO, VECTOR4_MAX, VECTOR4_ZERO } from '@fluxio/core/number/vector';
import { mustExist } from '@fluxio/core/check/must';
...
export const WindowFooter = ({ yes, no, cancel, confirm }: WindowFooterProps) => {
  ...
  return (
    <div {...c('Footer')}>
      {yes && <Button success icon={CheckIcon} title="Oui" onClick={controller.handle('yes')} />}
      {no && <Button warn icon={XCircleIcon} title="Non" onClick={controller.handle('no')} />}
      {cancel && (
        <Button error icon={BanIcon} title="Annuler" onClick={controller.handle('cancel')} />
      )}
      {confirm && (
        <Button success icon={CheckIcon} title="Valider" onClick={controller.handle('confirm')} />
      )}
    </div>
  );
};
